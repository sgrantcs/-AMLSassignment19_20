...
print("size of initial training set for gender is:", len(training), "images")
...
for item in training:
    ...
    if len(image) >0:
        ...
        training_labels.append(int(correct_labels[item][2])) #append correct labelfor gender to the training labels list
print("size of detected training set for gender is:", len(training_labels), "images")

fishface.train(training_data, np.array(training_labels)) #use openCV train function to train the model
print("the training for gender has finished")
...
print("size of detected predicted set for gender is:", len(prediction_data), "images")
...
for item, image in prediction_data:
        ...
        if pred == int(correct_labels[item][2]): #compare prediction value with correct value. 
...
with open('../Datasets/celeba/smiles_output.csv', 'w', newline='') as outfile: 
    ...
    for item, image in prediction_data:
        ...
        writer.writerow([item, correct_labels_row[1], new_prediction])
        label_index +=1
