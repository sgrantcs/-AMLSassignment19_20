...
#Ofishface = cv2.face.LBPHFaceRecognizer_create() # Alternative face classifier
...
for item in training:
    ...
    if len(image) >0:
        ...
        training_labels.append(int(correct_labels[item][3]))
...
for item in prediction:
    image = detect_face(os.path.join(images_dir, item)) #open image
    if len(image) >0:
        prediction_data.append(image) #append image array to training data list
